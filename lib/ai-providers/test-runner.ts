...
const mockTrend = {
  ...
  description:
    "Exploring how AI is revolutionizing video content creation with cutting-edge technology",
...
export async function testProviderInitialization() {
  ...
  try {
    ...
    console.log(
      `   Supported resolutions: ${veoProvider.getSupportedResolutions().join(", ")}\n`
    );
    ...
    console.log(
      `   Supported resolutions: ${runwayProvider.getSupportedResolutions().join(", ")}\n`
    );
    ...
    console.log(
      `   Supported resolutions: ${lumaProvider.getSupportedResolutions().join(", ")}\n`
    );
...
export async function testProviderFactory() {
  ...
  try {
    ...
    allProviders.forEach((provider) => {
      console.log(
        `   - ${provider.name} (priority: ${provider.getPriority()})`
      );
    });
    ...
    prioritizedProviders.forEach((provider, index) => {
      console.log(
        `   ${index + 1}. ${provider.name} (priority: ${provider.getPriority()})`
      );
    });
...
export function testScriptGeneration() {
  ...
  try {
    ...
    platforms.forEach((platform) => {
      ...
      console.log(
        `   ${platform}: ${script.aspectRatio}, max ${script.duration}s`
      );
    });
...
export async function testQueueManagement() {
  ...
  try {
    ...
    jobIds.forEach((jobId) => {
      ...
      if (job) {
        console.log(
          `   ${jobId.slice(-8)}: ${job.status} (${job.priority} priority, ${job.provider} provider)`
        );
      }
    });
...
export function testProgressTracking() {
  ...
  try {
    ...
    const progressListener = (jobId: string, update: any) => {
      progressUpdates++;
      console.log(
        `📊 Progress update for ${jobId.slice(-8)}: ${update.progress}% (${update.status})`
      );
    };
...
export async function runAllTests() {
  ...
  if (passedTests === totalTests) {
    console.log(
      "🎉 All tests passed! The AI video generation pipeline is ready."
    );
  } else {
